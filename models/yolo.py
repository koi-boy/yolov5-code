    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # 遍历backbone和head的每一层
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d,
            ...
            if c2 != no:
                ...
                c2 = make_divisible(c2 * gw, 8) #width_multiple  取大于等于c2 * gw且是divisor的最小倍数

            # 在初始arg的基础上更新 加入当前层的输入channel并更新当前层
            # [in_channel, out_channel, *args[1:]]   args[0]:c2: 当前层的输出的channel数(初定)
            args = [c1, c2, *args[1:]]
            ...
            args = [channel, re]
        #  elif m is SELayer:  # 加入SE模块 添加自定义模块 结构输入输出维度。
        else:
            # Upsample
            c2 = ch[f]  # args不变
        # -----------------------------------------------------------------------------------
        ...
        np = sum([x.numel() for x in m_.parameters()])  # number params  计算这一层的参数量  numel获取tensor中一共包含多少个元素
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):  # 对三个feature map分别进行处理
            ...
            if not self.training:
                ...
                y = x[i].sigmoid()
                # TODO： 这里讲的什么鸡巴玩意儿？inplace是干啥的？原地操作，节省运算内存，不用多储存变量y。
                # 例如nn.LeakyReLU(inplace=True) inplace=True的意思是进行原地操作，例如x=x+5，对x就是一个原地操作，y=x+5,x=y，完成了与x=x+5同样的功能但是不是原地操作，
...
class Model(nn.Module):
    # 这个模块是整个模型的搭建模块。且yolov5的作者将这个模块的功能写的很全，不光包含模型的搭建，还扩展了很多功能
    # 如：特征可视化，打印模型信息、TTA推理增强、融合Conv+Bn加速推理、模型搭载nms功能、autoshape函数：
    # 模型包含前处理、推理、后处理的模块(预处理 + 推理 + nms)。
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):
        ...
        # 创建网络模型
        # parse_model在Model模块的__init__函数中调用：
        ...
        if isinstance(m, Detect):
            ...
            # 计算三个feature map下采样的倍率  [8, 16, 32]  #TODO： x包含哪些维度？ [bs,c1,w,h]? [1, ch, s, s]
