...
# ============================================= 核心模块 =====================================
# autopad模块可以根据输入的卷积核计算该卷积模块所需的pad值。将会用于下面会讲到的Conv函数和Classify函数中。
def autopad(k, p=None):
    """
    用于Conv函数和Classify函数中
    根据卷积核大小k自动计算卷积核padding数（0填充） 
    v5中只有两种卷积：
       1、下采样卷积:conv3x3 s=2 p=（k-1）/2=k // 2 = 1 
...
class Conv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        """
        conv->bn->SILU
        基础组件,在Focus、Bottleneck、BottleneckCSP、C3、SPP、DWConv、TransformerBloc等模块中调用
        ...
        self.bn = nn.BatchNorm2d(c2)# TODO: nn.BatchNorm2d?
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        # TODO: 实验性 activation function 
...
class Focus(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        """
        这个模块是yolov5作者自己设计的一个模块，用在了模型的一开始，将输入图像先 slice 成4份，再做concat。
        ...
        # self.contract = Contract(gain=2)  #TODO: 实验性contract 也可以调用Contract函数实现slice操作

    def forward(self, x):

        # x(b,c,w,h) -> y(b,4c,w/2,h/2)  有点像做了个下采样
        return self.conv(torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1))# TODO: x[..., ::2, ::2] 步长为2的元素。2*2方格迭代输出
...
class C3(nn.Module):
    ...
    # 而作者之所以用C3来带代替BottleneckCSP也是有原因的，作者原话：
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])# TODO: nn.Sequential?
        # # TODO: 实验性 CrossConv 
...
class SPP(nn.Module):
    def __init__(self, c1, c2, k=(5, 9, 13)):
        """
        SPP模块的主要目的是为了将更多不同分辨率的特征进行融合，得到更多的信息。
        ...
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])# TODO: nn.ModuleList 与 nn.Sequential 区别？
...
# TODO： Contract函数改变输入特征的shape，将feature map的w和h维度(缩小)的数据收缩到channel维度上(放大)。如：x(1,64,80,80) to x(1,256,40,40)。
# TODO： Expand函数也是改变输入特征的shape，不过与Contract的相反， 是将channel维度(变小)的数据扩展到W和H维度(变大)。如：x(1,64,80,80) to x(1,16,160,160)。
...
# ============================================= 注意力机制 ======================================================
# 注意力机制主要是告诉模型那里更重要，什么更重要，并赋予其一个更大权重，而那些不重要的区域就赋予一个较小的权重，让模型更关注于重要的区域；
# 同时也会起到一个结合全局信息的作用。
# 关于注意力机制使用的一些个人看法，不一定正确：

# 1.在上采样+concat之后接一个注意力机制可能会更好？
# 2.channel-wise比spatial-wise更好用？
# 3.backbone结尾使用一个注意力机制？
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        b, _, w, h = x.shape
        p = x.flatten(2)
        p = p.unsqueeze(0) # TODO: unsqueeze
        p = p.transpose(0, 3) # TODO: transpose
        p = p.squeeze(3)        # TODO: squeeze
