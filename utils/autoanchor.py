...
def check_anchor_order(m):
    ...
    因为我们的m.anchors是相对各个feature map（每个feature map的感受野不同 检测的目标大小也不同 适合的anchor大小也不同）所以必须要顺序一致 
    否则效果会很不好。这个函数一般用于check_anchors最后阶段。
...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    # m: 从model中取出最后一层(Detect)   hasattr() 函数用于判断对象是否包含对应的属性。
...
# 使用遗传算法随机对anchors的wh进行变异，如果变异后效果变得更好
# （使用anchor_fitness方法计算得到的fitness（适应度）进行评估）就将变异后的结果赋值给anchors，如果变异后效果变差就跳过，默认变异1000次
