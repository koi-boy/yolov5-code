...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=False,
    ...
    # torch中采用了barrier()函数对其它非主进程进行阻塞，来达到同步的目的 https://blog.csdn.net/weixin_41041772/article/details/109820870
    ...
    #　[源码解析] PyTorch 分布式(1) --- 数据加载之DistributedSampler https://www.cnblogs.com/rossiXYZ/p/15142807.html
    ...
    dataloader = loader(dataset,
                        ...
                        sampler=sampler,
                        pin_memory=True,
                        collate_fn=LoadImagesAndLabels.collate_fn4 if quad else LoadImagesAndLabels.collate_fn)# TODO: collate_fn4 和 collate_fn是干啥的？
    return dataloader, dataset

class InfiniteDataLoader(torch.utils.data.dataloader.DataLoader):
    """ Dataloader that reuses workers
    当image_weights=False时（不根据图片样本真实框分布权重来选择图片）就会调用这两个函数 进行自定义DataLoader
    https://github.com/ultralytics/yolov5/pull/876
    使用InfiniteDataLoader和_RepeatSampler来对DataLoader进行封装, 代替原先的DataLoader, 能够永久持续的采样数据
    Uses same syntax as vanilla DataLoader
    """
    # TODO： class DataLoader解读
    # 图解 Python 编程(19) | 迭代器与生成器 https://www.showmeai.tech/article-detail/82
    # Python中的容器（container）、迭代器（iterator）和生成器（generator:yield） https://blog.51cto.com/u_13747676/3013993
    # 从零掌握迭代器及构建最简 DataLoader https://zhuanlan.zhihu.com/p/340465632
    ...
    def __iter__(self):
        for i in range(len(self)):
            yield next(self.iterator) #TODO： yield next？
class _RepeatSampler(object):
    ...
    def __iter__(self):
        while True:
            yield from iter(self.sampler) #TODO： iter 重载？
